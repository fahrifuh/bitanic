    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="float-end m-3">
                            <a href="{{ route('bitanic.hydroponic.user.create') }}" class="btn btn-icon btn-primary"
                                title="Tambah User">
                ...
                <div class="table-responsive text-nowrap">
                    <table class="table table-striped">
                        ...
                        <tbody class="table-border-bottom-0">
                            @forelse ($hydroponicUsers as $hydroponicUser)
                                <tr>
                                    <td>{{ ($hydroponicUsers->currentPage() - 1) * $hydroponicUsers->perPage() + $loop->iteration }}
                                    </td>
                                    <td>
                                        <a href="javascript:;" type="button" class="avatar pull-up"
                                            data-bs-toggle="modal" data-bs-target="#modalFoto"
                                            data-foto="{{ asset($hydroponicUser->picture ?? 'bitanic-landing/default-profile.png') }}"
                                    ...
                                    <td>{{ $hydroponicUser->username }}</td>
                                    <td>{{ $hydroponicUser->gender->getLabelText() }}</td>
                                    <td>{{ $hydroponicUser->created_at }}</td>
                                    <td>
                                        <a href="{{ route('bitanic.hydroponic.user.show', $hydroponicUser->id) }}"
                                            class="btn btn-info btn-icon btn-sm" title="Detail User">
                                            <i class="bx bx-list-ul"></i>
                                        </a>
                                        <a href="{{ route('bitanic.hydroponic.user.edit', $hydroponicUser->id) }}"
                                            class="btn btn-warning btn-icon btn-sm" title="Edit User">
                                            <i class="bx bx-edit"></i>
                                        </a>
                                        <button type="button" onclick="deletePest(this)"
                                            data-id="{{ $hydroponicUser->id }}"
                                            data-name="{{ $hydroponicUser->name }}"
                                            class="btn btn-danger btn-sm btn-icon" title="Hapus User">
                ...
                @if ($hydroponicUsers->hasPages())
                    <div class="row">
                        <div class="col-md-12">
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-center">
                                    {{ $hydroponicUsers->links() }}
                                </ul>
                            </nav>
                        </div>
                    </div>
