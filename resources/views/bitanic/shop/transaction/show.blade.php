<x-app-layout>
    ...
    <x-slot name="header">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Master / <a
                    href="{{ route('bitanic.shop.index') }}">Toko</a> / <a
                    href="{{ route('bitanic.shop.transaction-index') }}">Transaksi</a> /</span> Detail
            {{ $farmerTransactionShop->farmer_transaction->code }}</h4>
    </x-slot>
    {{-- End Header --}}
    ...
    <div class="row d-flex justify-content-center">
        <div class="col-12 col-md-6">
            ...
            <div class="card">
                <div class="card-body">
                    <h6>Order ID {{ $farmerTransactionShop->farmer_transaction->code }}</h6>
                    <h2>Total Rp&nbsp;{{ number_format_1($farmerTransactionShop->total) }}</h2>
                    ...
                    <div class="d-flex justify-content-between adivgn-items-center mb-1">
                        ...
                        <div>
                            <span @class([
                                'badge',
                                'bg-label-success' =>
                                    $farmerTransactionShop->farmer_transaction->status == 'settlement',
                                'bg-label-secondary' =>
                                    $farmerTransactionShop->farmer_transaction->status == 'pending',
                                'bg-label-danger' => in_array(
                                    $farmerTransactionShop->farmer_transaction->status,
                                    ['expire', 'cancel', 'deny', 'failure']),
                            ])>
                                {{ $farmerTransactionShop->farmer_transaction->status }}
                            </span>
                            <button class="btn btn-sm btn-icon btn-warning" onclick="statusUpdate(this.dataset)"
                                data-id="{{ $farmerTransactionShop->farmer_transaction->id }}">
                    ...
                    <div class="d-flex justify-content-between adivgn-items-center mb-3">
                        ...
                        <div>
                            <span @class([
                                'badge',
                                'bg-label-success' => $farmerTransactionShop->shipping_status == 2,
                                'bg-label-secondary' => $farmerTransactionShop->shipping_status == 0,
                                'bg-label-info' => $farmerTransactionShop->shipping_status == 1,
                            ])>
                                @if ($farmerTransactionShop->farmer_transaction->status == 'settlement')
                                    @switch($farmerTransactionShop->shipping_status)
                                        @case(0)
                                            Sedang Dikemas
                                        @break
                            ...
                            @if ($farmerTransactionShop->farmer_transaction->status == 'settlement' && $farmerTransactionShop->shipping_status == 0)
                                <button class="btn btn-sm btn-warning" data-bs-toggle="modal"
                                    data-bs-target="#modalResi" title="Klik untuk ubah status menjadi dikirim">Kirim</button>
                            @endif
                        </div>
                    </div>

                    <h5 class="border-bottom mb-1">Detail Harga</h5>
                    @foreach ($farmerTransactionShop->farmer_transaction_items as $farmer_transaction_item)
                        <div class="d-flex justify-content-between adivgn-items-center mb-1">
                    ...
                    <div class="d-flex justify-content-between adivgn-items-center mb-1">
                        ...
                        <span>
                            Rp&nbsp;{{ number_format_1($farmerTransactionShop->farmer_transaction->platform_fees) }}
                        </span>
                    </div>
                    <div class="d-flex justify-content-between adivgn-items-center mb-1">
                        <span>Biaya Pengiriman</span>
                        <span>
                            Rp&nbsp;{{ number_format_1($farmerTransactionShop->total_shipping) }}
                        </span>
                    </div>
                    @if ($farmerTransactionShop->discount)
                        <div class="d-flex justify-content-between adivgn-items-center mb-1">
                            <span>Diskon {{ $farmerTransactionShop->discount }}%</span>
                            <span>
                                -Rp&nbsp;{{ number_format_1(floor($farmerTransactionShop->subtotal * $farmerTransactionShop->transaction_item->quantity * ($farmerTransactionShop->discount / 100))) }}
                            </span>
                        </div>
                    @endif
                    <div class="d-flex justify-content-between adivgn-items-center border-top mb-3">
                        <span>Total</span>
                        <span>
                            Rp&nbsp;{{ number_format_1($farmerTransactionShop->total) }}
                        </span>
                    </div>
                    ...
                    <div class="d-flex flex-column align-items-start gap-1 mb-3">
                        <span><strong>Timestamp</strong>:
                            <br>{{ $farmerTransactionShop->farmer_transaction->created_at->formatLocalized('%d %B %Y') }}
                            {{ $farmerTransactionShop->farmer_transaction->created_at->format('H:i:s') }}</span>
                        <span><strong>Bank</strong>:
                            <br>{{ $farmerTransactionShop->farmer_transaction->bank_name }}</span>
                        <span><strong>Midtrans Token</strong>:
                            <br>{{ $farmerTransactionShop->farmer_transaction->midtrans_token }}</span>
                    </div>
                            ...
                            <div id="accordionTwo" class="accordion-collapse collapse" aria-labelledby="headingTwo"
                                ...
                                <div class="accordion-body d-flex flex-column align-items-start gap-2">
                                    <span><strong>Nama</strong>:
                                        <br>{{ $farmerTransactionShop->farmer_transaction->user?->name ?? '-' }}</span>
                                    <span><strong>Nomor Handphone</strong>:
                                        <br>{{ $farmerTransactionShop->farmer_transaction->user?->phone_number ?? '-' }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="card accordion-item">
                            ...
                            <div id="accordionThree" class="accordion-collapse collapse" aria-labelledby="headingThree"
                                data-bs-parent="#accordionExample">
                                <div class="accordion-body d-flex flex-column align-items-start gap-2">
                                    @if ($farmerTransactionShop->delivery_receipt)
                                        <span><strong>Resi</strong>: <br><span
                                                id="resi-text">{{ $farmerTransactionShop->delivery_receipt }}
                                    ...
                                    @endif
                                    <span><strong>Courier</strong>: <br>{{ $farmerTransactionShop->courier }}</span>
                                    <span><strong>Tipe</strong>: <br>{{ $farmerTransactionShop->type }}</span>
                                    <span><strong>Harga</strong>:
                                        <br>Rp&nbsp;{{ number_format($farmerTransactionShop->total_shipping, 0, ',', '.') }}</span>
                                    <span><strong>Nama Penerima</strong>:
                                        <br>{{ $farmerTransactionShop->farmer_transaction->user_recipient_name }}</span>
                                    <span><strong>Nomor Handphone Penerima</strong>:
                                        <br>{{ $farmerTransactionShop->farmer_transaction->user_recipient_phone_number }}</span>
                                    <span><strong>Alamat Penerima</strong>:
                                        <br>{{ $farmerTransactionShop->farmer_transaction->user_recipient_address }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="card accordion-item">
                                        ...
                                        <tbody>
                                            @foreach ($farmerTransactionShop->farmer_transaction_items as $farmer_transaction_item)
                                                <tr>
                                                    ...
                                                    <td>Rp&nbsp;{{ number_format($farmer_transaction_item->product_price, 0, ',', '.') }}</td>
                                                    <td>-Rp&nbsp;{{ number_format(floor($farmer_transaction_item->product_price * ($farmer_transaction_item->discount / 100)), 0, ',', '.') }}</td>
                                                    <td>Rp&nbsp;{{ number_format(($farmer_transaction_item->product_price - floor($farmer_transaction_item->product_price * ($farmer_transaction_item->discount / 100))) * $farmer_transaction_item->quantity, 0, ',', '.') }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
    ...
    @include('bitanic.shop.transaction._modal-resi', ['transaction_id' => $farmerTransactionShop->farmer_transaction->id])
    ...
    @push('scripts')
        ...
        <script>
            ...
            const statusUpdate = async ({
                ...
                const [data, error] = await yourRequest(
                    "{{ route('bitanic.shop.transaction-status-update', 'ID') }}".replace('ID',
                        id), settings
                )
