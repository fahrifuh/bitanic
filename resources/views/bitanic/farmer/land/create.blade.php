<x-app-layout>
    ...
    <x-slot name="header">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Master @if (Auth::user()->role == 'admin')
                ...
                @endif / {{ $farmer->full_name }} / <a
                    href="{{ route('bitanic.land.index', $farmer->id) }}">Data Lahan</a> / </span> Tambah</h4>
    </x-slot>
    {{-- End Header --}}
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('bitanic.land.store', ['farmer' => $farmer->id]) }}" method="POST"
                            ...
                            <div class="col mb-0">
                                ...
                                <div class="input-group">
                                    <input type="number" step="0.01" min="0" class="form-control"
                                        id="data-input-area" name="area" placeholder="0" aria-label="0"
                        ...
                        <div class="row g-2">
                            <div class="col mb-3">
                                ...
                                <input type="numeric" class="form-control" id="data-input-latitude" name="latitude"
                                    value="{{ old('latitude') }}" />
                            </div>
                            <div class="col mb-3">
                                <label for="data-input-longitude" class="form-label">Longitude</label>
                                <input type="numeric" class="form-control" id="data-input-longitude" name="longitude"
                                    value="{{ old('longitude') }}" />
                            </div>
                            <div class="col mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" min="0" step=".01" class="form-control"
                                        id="data-input-altitude" name="altitude" value="{{ old('altitude') }}"
                                        aria-describedby="basic-altitude" />
                        ...
                        <div class="row">
                            <div class="col mb-3">
                                <div class="bg-info text-white p-3 rounded" role="alert" id="alert-map">Klik 2 kali
                                    pada peta untuk set marker kebun.</div>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-12">
                                ...
                                <input class="form-control" id="data-input-color" type="color" name="color"
                                    value="#666EE8" />
                            </div>
                        </div>
                        <div class="row">
    ...
    @push('scripts')
        ...
        <script>
            ...
            map.on('dblclick', function(e) {
                ...
                fetch(`https://api.open-elevation.com/api/v1/lookup?locations=${stateData.lat},${stateData.lng}`)
                    .then(res => res.json())
                    .then(data => {
                        const altitude = data.results[0].elevation;
                        stateData.alt = altitude;

                        marker = L.marker([stateData.lat, stateData.lng]).bindPopup(`Lokasi`, {
                            closeButton: false
                        }).addTo(map).openPopup();
                        //   radiusMove();
                        fillForm()
                    ...
                    .catch(err => {
                        console.error('Gagal mendapatkan altitude: ', err);
                        fillForm();
                    })
            ...
            function fillForm() {
                ...
                $(`#data-input-altitude`).val(stateData.alt);
            }
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                marker = L.marker([-6.869080223722067, 107.72491693496704], {
                    draggable: false
                })

                polygon = L.polygon([-6.869080223722067, 107.72491693496704], {
                    color: 'red'
                })
