<x-app-layout>
    ...
    {{-- Header --}}
    <x-slot name="header">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Master / KTP /</span> Pengguna</h4>
    </x-slot>
    {{-- End Header --}}
                    ...
                    <tbody class="table-border-bottom-0">
                        @forelse ($farmers as $farmer)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('bitanic.ktp-farmer.ktp', $farmer->id) }}" target="__blank">Lihat KTP</a>
                                </td>
                                <td>
                                    <div class="d-flex align-item-center gap-1">
                                        <a
                                              href="{{ route('bitanic.farmer.edit', $farmer->id) }}"
                                              class="btn btn-success btn-icon btn-sm"
                                              title="Edit Kategori"
                                            >
                                            <i class="bx bx-check"></i>
                                        </a>
                                        <button type="button" onclick="deletePest(this)" data-id="{{ $farmer->id }}"
                                            data-name="{{ $farmer->name }}" class="btn btn-danger btn-sm btn-icon"
        ...
        <script>
            const acceptKtp = async e => {
                ...
                const [data, error] = await yourRequest(
                    "{{ route('bitanic.ktp-farmer.update', 'ID') }}".replace('ID',
                ...
                window.location = "{{ route('bitanic.ktp-farmer.index') }}"
            }
