<x-app-layout>
  ...
  {{-- Header --}}
  <x-slot name="header">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Master / <a href="{{ route('bitanic.device.index') }}">Data Perangkat</a> / <a href="{{ route('bitanic.v3-device.show', $device->id) }}">{{ $device->device_series }}</a> </span>/ Edit Selenoid {{ $selenoid->selenoid_id }}</h4>
  </x-slot>
  {{-- End Header --}}
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="card">
        <div class="card-body">
          <form action="{{ route('bitanic.v3-device.selenoid.update', ['device' => $device->id, 'selenoid' => $selenoid->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <!-- Validation Errors -->
                ...
                <div class="col mb-3">
                    ...
                    <select class="form-select select2-active" id="data-input-land-id" name="land_id" aria-label="Default select example">
                        ...
                        @forelse ($lands as $id => $land)
                            <option value="{{ $id }}" @if($id == $selenoid->land_id) selected @endif>{{ $land }}</option>
                        @empty
            ...
            <div class="row mb-3 g-2">
                ...
                <div class="col-12 col-md-6">
                    <div class="input-group">
                        <input type="number" min="0" max="60" class="form-control" name="water_before[minutes]" value="{{ $selenoid->selenoid_watering->water_before->minutes }}" />
                        <span class="input-group-text" id="basic-addon13">Menit</span>
                    </div>
                </div>
                <div class="col-12 col-md-6">
                    <div class="input-group">
                        <input type="number" min="0" max="60" class="form-control" name="water_before[seconds]" value="{{ $selenoid->selenoid_watering->water_before->seconds }}" />
                ...
                <div class="col-12 col-md-6">
                    <div class="input-group">
                        <input type="number" min="0" max="60" class="form-control" name="water_after[minutes]" value="{{ $selenoid->selenoid_watering->water_after->minutes }}" />
                        <span class="input-group-text" id="basic-addon13">Menit</span>
                    </div>
                </div>
                <div class="col-12 col-md-6">
                    <div class="input-group">
                        <input type="number" min="0" max="60" class="form-control" name="water_after[seconds]" value="{{ $selenoid->selenoid_watering->water_after->seconds }}" />
                        <span class="input-group-text" id="basic-addon13">Detik</span>
                    </div>
                </div>
            </div>
