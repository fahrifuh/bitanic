<x-app-layout>
    ...
    <x-slot name="header">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Pengaturan / <a
                    href="{{ route('bitanic.feedback-regular.index') }}">Ulasan User</a> /</span>
            {{ $feedback->user->name }}
        </h4>
    </x-slot>
    {{-- End Header --}}
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <div>
                            <h3 class="m-0">{{ $feedback->user->name }}</h3>
                        </div>
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-danger btn-sm" data-id="{{ $feedback->id }}"
                                data-name="{{ $feedback->user->name }}"
                                data-delete-url="{{ route('bitanic.feedback-regular.destroy', $feedback->id) }}"
                                data-redirect-url="{{ route('bitanic.feedback-regular.index') }}"
                    ...
                    <div class="d-flex flex-column align-items-start gap-2 mb-3">
                        <span><strong>Tanggal Dibuat</strong>:<br>
                            {{ now()->parse($feedback->created_at)->formatLocalized('%d %B %Y') }}</span>
                        <span><strong>Platform</strong>: <br>{{ $feedback->platform }}</span>
                        <span><strong>Ulasan</strong>: <br>{{ $feedback->reviews }}</span>
                    </div>
                </div>
            </div>
            <!--/ Striped Rows -->
